...
class RigidBody {
    constructor(object3D, mass = 1.0, restitution = 0.5, friction = 0.4) {
        ...
        this.orientation = object3D.quaternion.clone();
    ...
    integrate(deltaTime) {
        ...
        if (this.angularVelocity.lengthSq() > 0.0) {
            ...
            this.orientation.x += angularChange.x;
            this.orientation.y += angularChange.y;
            this.orientation.z += angularChange.z;
            this.orientation.w += angularChange.w;
            this.orientation.normalize();
        }

        // Apply to representation
        this.representation.position.copy(this.position);
        this.representation.quaternion.copy(this.orientation);
