...
export function getContacts(A, B) {
  ...
  switch (flippedPair) {
    ...
    case "sphere_box": {
      ...
      contacts.forEach(contact => contact.contactNormal);
      return contacts;
    }
  }
...
function getBVHBoxContact(A, B) {
  ...
  meshA.bvh.boundsTree.shapecast({
    ...
    intersectsTriangle: triIn => {
      ...
      box.clampPoint(triCenter, ptOnBox);
      ...
      if (distSq > 1e-8) {
        ...
        const contactPointWorld = ptOnTri.clone().applyMatrix4(meshA.matrixWorld);
        const normalMatrix = new THREE.Matrix3().getNormalMatrix(meshA.matrixWorld);
        const worldNormal = contactNormal.clone().applyMatrix3(normalMatrix).normalize();
  ...
  // Sort and keep the best 4 (deepest penetrations)
  contacts.sort((a, b) => b.penetrationDepth - a.penetrationDepth);
  const bestContacts = contacts.slice(0, 4);
...
function getSphereBoxContact(A, B) {
  const meshA = A.representation; // Sphere
  const meshB = B.representation; // Box

  const matrixToB = meshB.getMatrixToA(meshA);

  const sphere = meshA.sphere.clone().applyMatrix4(matrixToB);
  const sphereCenterLocal = sphere.center.clone();

  const box = meshB.box.clone(); // Box is AABB in local space
  const closestPointLocal = sphereCenterLocal.clone().clamp(box.min, box.max);

  const deltaLocal = new THREE.Vector3().subVectors(sphereCenterLocal, closestPointLocal);
  const distSq = deltaLocal.lengthSq();
  ...
  if (distSq < sphere.radius * sphere.radius) {
    ...
    // Avoid NaN normals
    const contactNormalLocal = dist > 0 ? deltaLocal.normalize() : new THREE.Vector3(0, 1, 0);

    // Convert contact point and normal back to world space
    const contactPointWorld = closestPointLocal.clone().applyMatrix4(meshB.matrixWorld);
    const contactNormalWorld = contactNormalLocal.clone().applyMatrix3(
      new THREE.Matrix3().getNormalMatrix(meshB.matrixWorld)
    ).normalize();

    const contact = {
      contactPoint: contactPointWorld,
      contactNormal: contactNormalWorld,
      penetrationDepth
    };
...
function getBoxContact(A, B) {
  ...
  const boxA = meshA.box.clone().applyMatrix4(meshA.matrixWorld);
  const boxB = meshB.box.clone().applyMatrix4(meshB.matrixWorld);

  // Check if boxes intersect
  if (!boxA.intersectsBox(boxB)) return [];

  // Generate corner points of boxB in world space
  const cornersB = getBoxCorners(meshB.box).map(corner =>
    corner.applyMatrix4(meshB.matrixWorld)
  );

  const contacts = [];

  for (const corner of cornersB) {
    const clamped = corner.clone().clamp(boxA.min, boxA.max); // Closest point on boxA
    const distSq = corner.distanceToSquared(clamped);
    if (distSq < 1e-8) continue;

    const penetrationDepth = Math.sqrt(distSq);
    const contactNormal = clamped.clone().sub(corner).normalize(); // Points from B to A
  ...
  // Sort contacts by depth (most deeply embedded first)
  contacts.sort((a, b) => b.penetrationDepth - a.penetrationDepth);

  const bestContacts = contacts.slice(0, 4);
  bestContacts.forEach(visualizeContact); // Optional
