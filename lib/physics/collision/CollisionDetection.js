...
function getBVHBoxContact(A, B) {
  ...
  const matrixToA = getMatrixToA(meshA,meshB);
  const obb = B.box.clone().applyMatrix4(matrixToA);
...
function getSphereContact(A, B) {
  ...
  const a = A.sphere.clone();
  const b = B.sphere.clone();
...
function getSphereBoxContact(A, B) {
  const sphere = A.sphere.clone().applyMatrix4(A.representation.matrixWorld);
  const sphereCenterWorld = sphere.center;
  const radius = sphere.radius;

  // Clone and transform OBB to world space
  const obb = B.box.clone().applyMatrix4(B.representation.matrixWorld);
...
function getBoxContact(A, B) {
  const obbA = A.box.clone().applyMatrix4(A.representation.matrixWorld);
  const obbB = B.box.clone().applyMatrix4(B.representation.matrixWorld);
