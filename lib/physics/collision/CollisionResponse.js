...
export function resolveCollision(A, B, contact) {
    ...
    if (A.mass !== 0.0) A.addImpulse(normalImpulse.clone().negate(),contactPoint);
    if (B.mass !== 0.0) B.addImpulse(normalImpulse,contactPoint);
    ...
    if (tangent.lengthSq() > 0.0001) {
        ...
        if (A.mass !== 0.0) A.addImpulse(frictionImpulse.clone().negate(),contactPoint);
        if (B.mass !== 0.0) B.addImpulse(frictionImpulse,contactPoint);
    }
}
...
export function applyPositionalCorrection(A, B, contact) {
    ...
    const allowedPenetration = 0.001; // tolerance slop
    const correctionPercent = 0.3;   // position correction
