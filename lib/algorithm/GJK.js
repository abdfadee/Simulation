...
export function GJK(shape1, shape2)
{
	...
	direction = support.point.clone().negate().normalize();
...
function handleLine(simplex, direction) {
	...
	if (sameDirection(ab, ao)) {
		...
		direction.copy(new THREE.Vector3().crossVectors(abXao, ab)).normalize();
	} else {
		simplex.splice(1); // Keep only point A
		direction.copy(ao).normalize();
	}
	...
	if (sameDirection(acPerp, ao)) {
		...
		if (sameDirection(abPerp, ao)) {
			...
			if (sameDirection(abc, ao)) {
				direction.copy(abc).normalize();
			} else {
				simplex.splice(0, simplex.length, A, C, B);
				direction.copy(abc).negate().normalize();
			}
		}
	}
